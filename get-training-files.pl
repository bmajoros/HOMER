...
##################################################################
# Generates files containing example sequences.
#
# verified   filename
# --------   --------
#    x       introns.fasta
...
#    x       donors.fasta
#    x       acceptors.fasta
#    x       upstream-intergenic.fasta
#    x       downstream-intergenic.fasta
#    x       start-codons.fasta
#    x       stop-codons.fasta
#    x       donor-frame-shifts.fasta
#    x       acceptor-frame-shifts.fasta
##################################################################
...
my $usage="$0 <*.gff> <*.fasta> <max-num-examples>";
die "$usage\n" unless @ARGV==3;
...
while(1)
  {
    last unless $intronCounter<$N || $exonCounter<$N || $donorCounter<$N ||
      $acceptorCounter<$N || $upStreamCounter<$N/2 || $downStreamCounter<$N
      || $startCounter<$N || $stopCounter<$N || $dfsCounter<$N || 
      $afsCounter<$N;
	...
	###################################################################
	# Process each exon and its surrounding subfeatures (introns,
	# splice junctions, and frame shifts)
	###################################################################
	for(my $i=0 ; $i<$numExons ; ++$i)
	  {
		...
		my $intronSeq=
		  substr($axisSequence,$intronBegin,$intronLength);
		if($strand eq "-") 
		  {$intronSeq=Translation::reverseComplement(\$intronSeq)}
		if($intronCounter<$N)
		  {
		    $fastaWriter->addToFasta(
					  ">$transcriptId /class=intron ".
					  "/begin=$intronBegin ".
					  "/end=$intronEnd".
					  "  /axis=$axisName",
					  $intronSeq,
		...
		if($strand eq "-")
		  {$acceptorSeq=
		     Translation::reverseComplement(\$acceptorSeq)}
		if($acceptorCounter<$N && length($acceptorSeq)>6)
		  {
		    $fastaWriter->addToFasta(
					">$transcriptId /class=acceptor ".
					"/begin=$acceptorBegin ".
					"/strand=$strand",
					$acceptorSeq,
					$fhAcceptors);
		...
		die $numCodons unless defined $lastCodon;
		my $exonLen=$exon->getLength();
		while($numCodons>0 && 
	...
	##################################################################
	  {
	    my ($intergenicSeq,$intergenicBegin,$intergenicEnd,
		$intergenicLen);
	    if($strand eq "+")
	      {
		my $prevTranscriptThreePrime=
		  ($i>0 ? $transcripts->[$i-1]->{threePrime} 
		   : $startCodon->{absoluteCoord}-500);
		if($prevTranscriptThreePrime<0) 
		  {$prevTranscriptThreePrime=0}
		$intergenicBegin=
		  #$prevTranscript->{threePrime}+3; # skip stop codon
		  $prevTranscriptThreePrime+3;
		...
		my $upStream=substr($intergenicSeq,$upstreamBegin,
				    $halfLength);
